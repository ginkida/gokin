...
func TestShouldAttemptStagnationRecovery(t *testing.T) {
	...
	cases := []struct {
		...
		{"read attempt 3 enters finalize phase", read, 3, true},
		{"read attempt 4 second finalize", read, 4, true},
		{"read attempt 5 everything spent", read, 5, false},
		{"edit attempt 0", edit, 0, true},
		{"edit attempt 1 budget spent (no finalize for mutations)", edit, 1, false},
		{"bare bash gets no recovery (no command arg = not read-only)", bash, 0, false},
