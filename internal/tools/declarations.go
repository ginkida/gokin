	...
	return &genai.FunctionDeclaration{
		...
		Parameters: &genai.Schema{
			...
			Properties: map[string]*genai.Schema{
				...
				"timeout_seconds": {
					Type:        genai.TypeInteger,
					Description: "Foreground command timeout in seconds (1-1800)",
				},
				"run_in_background": {
					Type:        genai.TypeBoolean,
...
func GetAllDeclarations() map[string]*genai.FunctionDeclaration {
	return getAllDeclarationsForWorkDir("")
}

// getAllDeclarationsForWorkDir builds lazy/model-facing schemas from the same
// live Declaration methods used after instantiation. The previous parallel set
// of hand-maintained schemas had drifted for dozens of tools (different names,
// required fields and types), so models planned calls that runtime validation
// could never accept. Constructors here are side-effect-free; tools themselves
// are still created lazily by DefaultLazyRegistry.
func getAllDeclarationsForWorkDir(workDir string) map[string]*genai.FunctionDeclaration {
	return map[string]*genai.FunctionDeclaration{
		"read":                 NewReadTool(workDir).Declaration(),
		"write":                NewWriteTool(workDir).Declaration(),
		"edit":                 NewEditTool(workDir).Declaration(),
		"bash":                 NewBashTool(workDir).Declaration(),
		"glob":                 NewGlobTool(workDir).Declaration(),
		"grep":                 NewGrepTool(workDir).Declaration(),
		"todo":                 NewTodoTool().Declaration(),
		"list_dir":             NewListDirTool(workDir).Declaration(),
		"diff":                 NewDiffTool(workDir).Declaration(),
		"tree":                 NewTreeTool(workDir).Declaration(),
		"env":                  NewEnvTool().Declaration(),
		"ask_user":             NewAskUserTool().Declaration(),
		"task_output":          NewTaskOutputTool().Declaration(),
		"task_stop":            NewTaskStopTool().Declaration(),
		"web_fetch":            NewWebFetchTool().Declaration(),
		"web_search":           NewWebSearchTool().Declaration(),
		"task":                 NewTaskTool().Declaration(),
		"kill_shell":           NewKillShellTool().Declaration(),
		"memory":               NewMemoryTool().Declaration(),
		"enter_plan_mode":      NewEnterPlanModeTool().Declaration(),
		"update_plan_progress": NewUpdatePlanProgressTool().Declaration(),
		"get_plan_status":      NewGetPlanStatusTool().Declaration(),
		"exit_plan_mode":       NewExitPlanModeTool().Declaration(),
		"undo_plan":            NewUndoPlanTool().Declaration(),
		"redo_plan":            NewRedoPlanTool().Declaration(),
		"batch":                NewBatchTool(workDir).Declaration(),
		"refactor":             NewRefactorTool().Declaration(),
		"check_impact":         NewCheckImpactTool(workDir).Declaration(),
		"memorize":             NewMemorizeTool(nil).Declaration(),
		"tools_list":           NewToolsListTool(nil).Declaration(),
		"request_tool":         NewRequestToolTool().Declaration(),
		"ask_agent":            NewAskAgentTool().Declaration(),
		"copy":                 NewCopyTool(workDir).Declaration(),
		"move":                 NewMoveTool(workDir).Declaration(),
		"delete":               NewDeleteTool(workDir).Declaration(),
		"mkdir":                NewMkdirTool(workDir).Declaration(),
		"git_log":              NewGitLogTool(workDir).Declaration(),
		"git_blame":            NewGitBlameTool(workDir).Declaration(),
		"git_diff":             NewGitDiffTool(workDir).Declaration(),
		"git_status":           NewGitStatusTool(workDir).Declaration(),
		"git_add":              NewGitAddTool(workDir).Declaration(),
		"git_commit":           NewGitCommitTool(workDir).Declaration(),
		"ssh":                  NewSSHTool().Declaration(),
		"coordinate":           NewCoordinateTool().Declaration(),
		...
		"run_tests":            NewRunTestsTool(workDir).Declaration(),
		"git_branch":           NewGitBranchTool(workDir).Declaration(),
		"git_pr":               NewGitPRTool(workDir).Declaration(),
		"mcp_admin":            NewMCPAdminTool().Declaration(),
		"loop_control":         NewLoopControlTool().Declaration(),
		"review_changes":       NewReviewChangesTool(workDir).Declaration(),
		"go_to_definition":     NewGoToDefinitionTool(workDir).Declaration(),
		"find_references":      NewFindReferencesTool(workDir).Declaration(),
		"go_search":            NewGoSearchTool(workDir).Declaration(),
	}
}
