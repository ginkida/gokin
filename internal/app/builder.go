...
func (b *Builder) wireDependencies() error {
	...
	// Executor handler = shared bookkeeping + TUI presenter. The shared
	// half (journal, heartbeat, response metadata, plan-step effects) lives
	// in buildExecutionHandler so headless and interactive modes can't
	// drift — see agent_events.go.
	app.setPresenter(&tuiPresenter{app: app})
	b.executor.SetHandler(app.buildExecutionHandler(b.projectMemory))
	...
	// Unified sink: journals sub-agent tool events (with agent_id) and
	// forwards to the active presenter — see agent_events.go.
	b.agentRunner.SetOnSubAgentActivity(app.handleSubAgentActivity)
