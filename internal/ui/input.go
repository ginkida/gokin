...
func defaultCommands() []CommandInfo {
	return []CommandInfo{
		...
		{Name: "oauth-login", Description: "Login via OAuth (Gemini or OpenAI)", Category: "Auth",
			Args:  []ArgInfo{{Name: "provider", Required: false, Type: "option", Options: []string{"gemini", "openai"}}},
			Usage: "/oauth-login [gemini|openai]"},
		{Name: "oauth-logout", Description: "Remove OAuth credentials", Category: "Auth",
			Args:  []ArgInfo{{Name: "provider", Required: false, Type: "option", Options: []string{"gemini", "openai"}}},
			Usage: "/oauth-logout [gemini|openai]"},
