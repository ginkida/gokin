...
// in the system block instead re-billed the whole system+tools prefix on
// every working-memory change (roadmap #7). Mutable post-construction like
// EnableThinking — guarded by c.mu, snapshot before use in send paths.
...
// both are normalized to a block array. Non-user final messages (shouldn't
// happen in practice) are left untouched rather than risking a malformed
// conversation.
func appendTurnContextBlock(messages []map[string]any, turnContext string) []map[string]any {
	...
	case []any:
		last["content"] = append(content, any(ctxBlock))
	default:
		// Unknown content shape — don't touch it.
	}
	return messages
}
...
//
// Verified by live request-usage inspection (April 2026):
//   - Anthropic native: honours cache_control, reports both
//     cache_creation_input_tokens and cache_read_input_tokens.
//   - MiniMax: same as Anthropic.
//   - Moonshot Developer API / Kimi Coding Plan (api.kimi.com/coding):
//     honours cache_control.
//   - DeepSeek (api.deepseek.com/anthropic): caches automatically on
//     the server side. cache_control markers are accepted (don't 400)
//     and server reports cache_read_input_tokens on repeat prefixes —
//     measured 95% input-token reduction on a 1.2K-token prefix on the
...
func mergeConsecutiveMessages(messages []map[string]any) []map[string]any {
	...
	for i := 1; i < len(messages); i++ {
		...
		if prevRole == currRole {
			...
			if prevRole == "user" {
				prev["content"] = orderToolResultsFirst(extractContentArray(prev["content"]))
			}
