...
// request. buildAssistantMessage DROPS unsigned thinking parts from every
// outgoing request, but the old estimator walked the raw parts and counted
// them anyway — a thinking-heavy session's context bar was systematically
// inflated by thousands of tokens the provider never sees.
func TestEstimateTokens_ExcludesDroppedUnsignedThinking(t *testing.T) {
	...
	// The unsigned thinking never reaches the provider, so the two histories
	// must estimate (nearly) identically — before the fix the delta was
	// ~11K chars / 3.5 ≈ 3K tokens.
	if diff := inflated.TotalTokens - plain.TotalTokens; diff > 50 {
		t.Fatalf("unsigned thinking inflated the estimate by %d tokens (plain=%d, inflated=%d)",
			diff, plain.TotalTokens, inflated.TotalTokens)
	}
