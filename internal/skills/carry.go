...
// IsSyntheticCarryText reports whether text is a complete, strictly valid
// active-skill carry envelope generated by this package. Callers that derive
// user-visible metadata from conversation history can use it to ignore the
// internal control message without treating marker-looking ordinary text as
// synthetic.
func IsSyntheticCarryText(text string) bool {
	_, ok := parseCarryEnvelope(text)
	return ok
}
...
func isSyntheticCarryContent(content *genai.Content) bool {
	...
	return IsSyntheticCarryText(part.Text)
}
